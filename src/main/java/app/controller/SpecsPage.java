...
public class SpecsPage extends GenericServlet {
    ...
    public void service(ServletRequest request, ServletResponse response)
        ...
        resp.setHeader("Cache-Control","no-cahe, no-store, must-revalidate");
        ...
        if (session == null || session.getAttribute("userAuthenticated") == null) {
            ...
            resp.sendRedirect("login?dest=specs");
            return;
        }
...
//        HttpSession session = httpReq.getSession(true);
        ...
        session.setAttribute("lastViewed","Range Rover Sport");
